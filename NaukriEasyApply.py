...
originalResumePath = R"C:\Users\Admin\Downloads\Documents\WASIM_AKHTAR_VisualCV_Resume.pdf"
# Add Path where modified resume should be saved
modifiedResumePath = R"C:\Users\Admin\Downloads\Documents\WASIM_AKHTAR_VisualCV_Resume.pdf"

# Update your naukri username and password here before running
username = "Wasim.akh2@gmail.com "
password = "System@6"
mob = "9718742936"  # Type your mobile number here
...
def SearchAJob(driver,keyword,locationstr):
    print (keyword)
    try:
        urlList=[]
        log_msg("----Strtin Job Search -----")
        # jobSearchURL="https://www.naukri.com/mnjuser/homepage"
        jobSearchURL="https://www.naukri.com/"+str(keyword)+"-jobs-in-"+str(locationstr)
        log_msg(jobSearchURL)
        driver.get(jobSearchURL)
        jobListClass="jobTuple bgWhite br4 mb-8"
        applybutton=".apply-button-container > .waves-ripple"
        applybuttonXpath="//*[@id='root']/main/div[2]/div[2]/section[1]/div[1]/div[3]/div/button[2]"

        # By.CLASS_NAME
        JobListElements=driver.find_elements(By.CSS_SELECTOR,".title")
        for child in JobListElements:
            # if "jdUrl" in child.get_attribute("id"):
            urlList.append(str(child.get_attribute("href")))
        ...
        for atag in urlList:
            try:
                ...
                time.sleep(1)
                ael=GetElement(driver,applybuttonXpath,locator="XPATH")
                ael.click()
                time.sleep(3)
        ...
        # skillKeywordSearchID="qsb-keyskill-sugg"
        # LocationSearchID="qsb-location-sugg"
        # searchButtonClass=".search"
        # driver.get(jobSearchURL)
        # KeySkill=GetElement(driver,skillKeywordSearchID,locator="ID")
        # KeySkill.click()
        # KeySkill.send_keys(keyword)
        # time.sleep(1)
        # Location=GetElement(driver,LocationSearchID,locator="ID")
        # Location.click()
        # Location.send_keys(locationstr)
        # time.sleep(1)
        # SearchButton=GetElement(driver,searchButtonClass,locator="CSS_SELECTOR")
        # SearchButton.click()

        log_msg("----End Job Search -----")
...
def main():
    ...
    driver = None
    try:
        status, driver = naukriLogin(false)
        if status:
